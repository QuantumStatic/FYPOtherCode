...
PARAM = None
NORMAL_DECORATOR = False
def execute_this(func=None, /, **kwargs) -> tuple[str, Any]:
    """
    Set "normal=True" to use this like a normal decorator\n
    Set "args=[your, args]" pass arguments to the function
    """
    global PARAM, NORMAL_DECORATOR

    if func is None:
        if any(kwargs):
            NORMAL_DECORATOR = kwargs.setdefault("normal", False)
            PARAM = kwargs.setdefault("args", None)
        return execute_this

    def normal_decorator(func) -> Callable[[], tuple[str, Any]]:
        def wrapper(*input_args) -> tuple[str, Any]:
            nonlocal func
            if any(input_args):
                execute_this(args=input_args)
            return execute_this(func)
        return wrapper
    ...
    if not callable(func): #isinstance(func, function) and func is not types.FunctionType and func is not types.BuiltinFunctionType and func is not types.BuiltinMethodType:
        raise TypeError(f"A function for execution was expected but got class {type(func).__name__} instead.")
